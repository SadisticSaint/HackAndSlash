...
public class UICharacterSelectionMarker : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (!IsLockedIn)
        {
            if (player.Controller.horizontal > 0.1f)
            {
                MoveToCharacterPanel(menu.RightPanel);
            }
            else if (player.Controller.horizontal < -0.1f)
            {
        ...
        {
            if (player.Controller.attackPressed) //make so only player one can start game
            {
                menu.TryToStartGame();
            }
        }
